...
/* $Id: mcrypt_extra.c,v 1.23 2002/07/06 10:18:18 nmav Exp $ */
...
void *mcrypt_dlopen(mcrypt_dlhandle *handle, const char *a_directory, const char *m_directory,
		    const char *filename);
...
WIN32DLL_DEFINE char **mcrypt_list_algorithms(const char *libdir, int *size)
{
	DIR *pdir;
	char directory[512];
	char *dirname;
	...
	*size = 0;
	...
	while( mps[i].name!=0 || mps[i].address!=0) {   
		if (mps[i].name!=NULL && mps[i].address==NULL) {
			if (mcrypt_algorithm_module_ok( mps[i].name, NULL) > 0) {
				filename =
				    realloc(filename,
					    ((*size) + 1) * sizeof(char *));
				if (filename==NULL) {
				...
				if (filename[*size]==NULL) goto freeall;
				(*size)++;
			}
		}
		i++;
		...
		if (dirname != NULL) {
			...
			if (tmpsize > 3) {
				if (mcrypt_algorithm_module_ok
					...
					if (_mcrypt_search_symlist_lib( dirname)!=NULL) {
						free(dirname);
 ...
 */
const char *
mcrypt_check_version( const char *req_version )
{
